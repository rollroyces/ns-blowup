...
formalization of the spectral NS scheme: \textbf{137 proved theorems across 9 files}
...
sufficient condition for NS regularity, plus (vii)~\textbf{the Constantin--Iyer
alignment property} (22 theorems) formalizing the vorticity--strain alignment
angle $\theta$ as a spectral object with provable bound $\theta \le \pi$,
(viii)~\textbf{the Cao--Titi directional regularity criterion} (19 theorems)
with the Lean proof that BdV $\Rightarrow$ CT (directional regularity is
implied by full-gradient regularity), (ix)~\textbf{the Tao 2016 no-go
interface} (5 theorems + 1 axiom) axiomatizing that any regularity proof using
only harmonic analysis + energy identity is impossible, (x)~\textbf{the
unified-composite theorem} (17 theorems) showing the three criteria unify
into a single statement with forward, reverse, and round-trip hierarchy,
and (xi)~\textbf{the vortex-stretching identity} (16 theorems + 1 axiom)
formalizing the discrete algebraic identity
$\partial_t \omega = \Delta \omega + \omega\!\cdot\!\nabla u - (\nabla u)^T\!\omega$
on the truncated spectral scheme---the geometric ``finer structure'' that
Tao's averaging breaks, plus (xii)~\textbf{the bridge theorem} (17 theorems + 2 axioms)
combining the vortex-stretching identity with a Serrin-type integrability
estimate (the Beirao da Veiga-style gradient bound) to derive that
``vortex-stretching identity holds + bounded H$^1$ energy'' implies
smoothness of the trajectory on $[0,T]$. The hierarchy theorem
...
are proved in Lean. The Clay Millennium problem remains
...
NS scheme, three formal regularity criteria (Beirao da Veiga,
Constantin--Iyer, Cao--Titi), their composite/hierarchy/unified
structure, the Tao 2016 no-go interface, and the vortex-stretching
identity. Specifically, the Lean component comprises
\textbf{137 proved theorems across 9 files} (0 sorries, 4 honest axioms),
totalling \textbf{4790 lines}:
\begin{itemize}
...
\item \texttt{VortexStretching.lean} (479 lines, 16 theorems + 1 axiom):
  ...
  ``finer structure'' that Tao's averaging breaks, and it is the formal
  counterpart of the candidate insight that \emph{alignment arises from
  the vortex-stretching identity}, not from energy identity + harmonic
  analysis alone.
\item \texttt{VortexRegularity.lean} (582 lines, 17 theorems + 2 axioms):
  ...
  the energy identity. The two axioms
  (\texttt{vortEnergyS\_le\_const\_h1} and \texttt{per\_mode\_cross\_bound})
  are honest external inputs---the algebraic skeleton that the
  cross-product bound implies $\norm{\omega}^2 \le C \cdot \norm{\nabla u}^2$,
  which is the bridging fact any regularity proof would need.
\end{itemize}
...
that any future regularity proof would need. The full chain---Leray
energy + BdV + CI + CT + Composite + Tao no-go + Unified + vortex-
stretching identity + bridge theorem---is now formalized in Lean,
totalling 137 theorems across 9 files with 0 sorries.
...
constructions of wild solutions. This work formalizes the first three
in Lean and proves their hierarchical and composite structure. The
\textbf{vortex-stretching identity} (VortexStretching.lean) is the
formal counterpart of the candidate insight that
\emph{alignment is a consequence of the vortex-stretching geometry},
not an independent structural property. Identifying which of these
(or which combination) constitutes the ``finer structure'' required by
